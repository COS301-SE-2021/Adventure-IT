    ...
    public List<ViewBudgetResponse> viewBudget(UUID id) throws Exception {
        ...
        for (BudgetEntry entry:entries) {
            if(entry instanceof UTUExpense){
                list.add(new ViewBudgetResponse(entry.getId(),entry.getEntryContainerID(),entry.getAmount(),entry.getTitle(),entry.getDescription(),entry.getCategory(),((UTUExpense) entry).getPayee(),entry.getPayer()));
            }
            else{
                list.add(new ViewBudgetResponse(entry.getId(),entry.getEntryContainerID(),entry.getAmount(),entry.getTitle(),entry.getDescription(),entry.getCategory(),((UTOExpense) entry).getPayee(),entry.getPayer()));
            }
        }
...
//  @Transactional
    ...
    public String restoreBudget(UUID id,UUID userID) throws Exception {
